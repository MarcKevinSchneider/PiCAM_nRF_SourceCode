/*
 * Script Function: Overall conguration for the nRF
 *
 * This script was ported from the original Yang et al. 2023 script. 
 */
...
float getBattV() {
  // CONFIRMED values from Seeed schematic + official wiki:
  //   READ_BAT_ENABLE (D14 / P0.14) must be pulled LOW to enable the divider.
  //   BATT_SENSE_PIN  (PIN_VBAT / P0.31 / AIN7) is the ADC input.
  //   Divider: 1510k / 510k  =>  scale factor = 1510.0 / 510.0 = ~2.961
  //   ADC reference: AR_INTERNAL_3_0 = 3.0V, 12-bit resolution.
  //
  // WARNING: do NOT set READ_BAT_ENABLE HIGH while charging -- P0.31 can be
  // damaged if it exceeds 3.6V. Always disable (HIGH) only after reading.

  pinMode(READ_BAT_ENABLE, OUTPUT);
  digitalWrite(READ_BAT_ENABLE, LOW);   // enable voltage divider

  analogReference(AR_INTERNAL_3_0);
  analogReadResolution(12);

  float measuredvbat = analogRead(BATT_SENSE_PIN);
  measuredvbat = measuredvbat * (3.0f / 4096.0f) * (1510.0f / 510.0f);

  digitalWrite(READ_BAT_ENABLE, HIGH);  // disable divider to save power
  return measuredvbat;
}
